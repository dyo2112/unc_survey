...
class ProsecutorAnalyzer:
    ...
    def run_all_analyses(self):
        ...
        # Print sample sizes for transparency - COUNT ACTUAL MATCHES
        print(f"\nSample sizes:")
        print(f"  All prosecutors: {len(self.df_prosecutors)}")
        print(f"  Filtered (≥{Config.MIN_RATINGS_THRESHOLD} ratings): {len(self.df_prosecutors_filtered)}")
        
        # Count actual matches
        matched_count = int(self.df_matched_all['election_year'].notna().sum())
        filtered_matched_count = int(self.df_matched['election_year'].notna().sum())
        print(f"  Full matched (with election data): {matched_count}/{len(self.df_matched_all)}")
        print(f"  Filtered + matched: {filtered_matched_count}/{len(self.df_matched)}")
        
        # Count by notability AMONG MATCHED ONLY
        matched_only = self.df_matched_all[self.df_matched_all['election_year'].notna()]
        notable_matched = matched_only[matched_only['is_notable'] == True]
        state_matched = matched_only[matched_only['is_notable'] == False]
