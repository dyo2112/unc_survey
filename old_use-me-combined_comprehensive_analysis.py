...
class ComprehensiveProsecutorAnalyzer:
    ...
    def analyze_state_patterns(self):
        ...
        if len(self.state_df) > 0:
            ...
            notable_mean = self.da_df['Mean_Score'].mean()
            state_mean = self.state_df['mean_score'].mean()
            
            print(f"\n📊 NOTABLE VS STATE COMPARISON:")
            print(f"   Notable DAs mean: {notable_mean:.2f}")
            print(f"   State DAs mean: {state_mean:.2f}")
            print(f"   Gap: {notable_mean - state_mean:.2f} points")
    ...
    def create_all_visualizations(self, output_dir='output'):
        ...
        # VIZ 1b: Progressive vs Traditional comparison
        self.create_viz_progressive_vs_traditional(output_path)
        ...
        # VIZ 4b: Simple mean comparison (Notable vs State)
        if hasattr(self, 'state_df') and len(self.state_df) > 0:
            self.create_viz_simple_comparison(output_path)
        
        # VIZ 5: Electoral transitions (bar chart style)
        if hasattr(self, 'transitions_df'):
            self.create_viz_transitions_bars(output_path)
        
        # VIZ 5b: Electoral transitions (change visualization)
        if hasattr(self, 'transitions_df'):
        ...
        # VIZ 8: Notable vs State comparison
        if hasattr(self, 'state_df') and len(self.state_df) > 0:
            self.create_viz_notable_vs_state(output_path)
        
        # VIZ 9: National vs State comparison (Revised 4-Panel)
        if hasattr(self, 'state_df') and len(self.state_df) > 0:
            self.create_viz_national_vs_state_revised(output_path)
    ...
    def create_viz_progressive_vs_traditional(self, output_path):
        """Create progressive vs traditional comparison (from complete_analysis_script)"""
        print("   Creating Viz 1b: Progressive vs Traditional Comparison...")
        
        fig, ax = plt.subplots(figsize=(12, 10))
        
        reliable = self.da_df[self.da_df['Substantive_Ratings'] >= 20].copy()
        top_prog = reliable.nlargest(10, 'Mean_Score')
        top_trad = reliable.nsmallest(10, 'Mean_Score')
        
        # Reverse order so traditional are on top, progressive on bottom
        combined = pd.concat([top_trad[::-1], top_prog[::-1]])
        
        # Color by threshold (3.5 for 1-4 scale, equivalent to 3.5 on 1-5 scale)
        ...
        # Add vertical line at midpoint (2.5 for 1-4 scale)
        ax.axvline(x=2.5, color='gray', linestyle='--', alpha=0.5, linewidth=1)
        
        # Labels and title
        ax.set_xlabel('Progressiveness Score (1=Very Traditional, 4=Very Progressive)', fontsize=12)
        
        # Calculate total respondents for subtitle
        total_respondents = len(self.df_consented)
        ...
        # Adjust layout
        plt.tight_layout()
        plt.savefig(output_path / 'viz_1b_progressive_vs_traditional.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def create_viz_simple_comparison(self, output_path):
        """Create simple bar chart comparing Notable vs State prosecutor means"""
        print("   Creating Viz 4b: Simple Mean Comparison...")
        
        reliable_notable = self.da_df[self.da_df['Substantive_Ratings'] >= self.min_ratings]
        notable_mean = reliable_notable['Mean_Score'].mean()
        notable_std = reliable_notable['Mean_Score'].std()
        
        state_mean = self.state_df['mean_score'].mean()
        state_std = self.state_df['mean_score'].std()
        
        fig, ax = plt.subplots(figsize=(10, 8))
        
        categories = ['50 Notable DAs\n("Famous")', 'State-Level DAs\n(Local)']
        means = [notable_mean, state_mean]
        stds = [notable_std, state_std]
        
        bars = ax.bar(categories, means, yerr=stds, capsize=10,
                     color=['#e74c3c', '#3498db'], alpha=0.7, edgecolor='black', linewidth=2)
        
        ax.set_ylabel('Mean Progressiveness Score', fontsize=13, fontweight='bold')
        ax.set_title('The Two Prosecutor Worlds: Notable vs State-Level DAs', 
                    fontsize=15, fontweight='bold', pad=20)
        ax.set_ylim(1, 4)
        ax.axhline(y=2.5, color='gray', linestyle='--', alpha=0.5, linewidth=1.5, label='Midpoint (2.5)')
        ax.grid(axis='y', alpha=0.3)
        ax.legend()
        ...
        ax.annotate('', xy=(0.5, means[0]-0.05), xytext=(0.5, means[1]+0.05),
                   arrowprops=dict(arrowstyle='<->', color='red', lw=2.5))
        gap = means[0] - means[1]
        gap_pct = (gap / 3) * 100  # 3 is the range of scale
        ax.text(0.6, np.mean(means), f'Δ = {gap:.2f}\n({gap_pct:.0f}% of scale)',
               color='red', fontsize=12, fontweight='bold',
               bbox=dict(boxstyle='round,pad=0.5', facecolor='white', alpha=0.9))
        
        plt.tight_layout()
        plt.savefig(output_path / 'viz_4b_simple_comparison.png', dpi=300, bbox_inches='tight')
        plt.close()
    
    def create_viz_transitions_bars(self, output_path):
        """Create bar chart style transitions visualization"""
        print("   Creating Viz 5a: Electoral Transitions (Bar Chart)...")
        ...
        x = np.arange(len(transitions_sorted))
        width = 0.35
        
        bars1 = ax.bar(x - width/2, transitions_sorted['Pred_Score'].values, width,
                      label='Predecessor', color='#3498db', alpha=0.8, edgecolor='black')
        bars2 = ax.bar(x + width/2, transitions_sorted['Succ_Score'].values, width,
                      label='Successor', color='#e74c3c', alpha=0.8, edgecolor='black')
        
        # Add change arrows and labels
        for i, (idx, row) in enumerate(transitions_sorted.iterrows()):
            y1 = row['Pred_Score']
            y2 = row['Succ_Score']
            color = '#2ecc71' if y2 > y1 else '#e74c3c'
        ...
        ax.set_xticks(x)
        ax.set_xticklabels([f"{row['Jurisdiction']}\n{row['Predecessor']} → {row['Successor']}"
                           for _, row in transitions_sorted.iterrows()],
                          rotation=45, ha='right', fontsize=9)
        ax.set_ylabel('Progressiveness Score (1-4 Scale)', fontsize=12, fontweight='bold')
        ax.set_title('Prosecutor Transitions: Ideological Shifts (2022-2024)',
                    fontsize=14, fontweight='bold', pad=15)
        ax.axhline(y=2.5, color='gray', linestyle='--', alpha=0.5, linewidth=1.5, label='Midpoint')
        ax.legend(loc='upper left', fontsize=11)
        ax.set_ylim(1, 4)
        ax.grid(axis='y', alpha=0.3)
        
        plt.tight_layout()
        plt.savefig(output_path / 'viz_5a_transitions_bars.png', dpi=300, bbox_inches='tight')
        plt.close()
    
    def create_viz_transitions(self, output_path):
        """Create jurisdiction transitions change visualization"""
        print("   Creating Viz 5b: Electoral Transitions (Change View)...")
        ...
        plt.tight_layout()
        plt.savefig(output_path / 'viz_5b_transitions.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def create_viz_geographic(self, output_path):
        ...
        ax1.set_xlabel('Number of Prosecutors in Notable Sample', fontweight='bold')
    ...
    def create_viz_notable_vs_state(self, output_path):
        """Create notable vs state comparison visualization"""
        print("   Creating Viz 8: Notable vs State Comparison...")
        ...
        ax1.hist(reliable_notable['Mean_Score'], bins=20, alpha=0.6,
                color='#e74c3c', label=f'Notable DAs (μ={notable_mean:.2f})', edgecolor='black')
        ...
        ax1.set_title('Score Distribution: Notable vs State', fontweight='bold', fontsize=13)
        ax1.legend()
        ax1.grid(alpha=0.3)
        ax1.set_xlim(1, 4)
        ...
        # National camps
        notable_prog = len(reliable_notable[reliable_notable['Mean_Score'] >= 3])
        notable_trad = len(reliable_notable[reliable_notable['Mean_Score'] <= 2])
        notable_mod = len(reliable_notable[(reliable_notable['Mean_Score'] > 2) & 
                                        (reliable_notable['Mean_Score'] < 3)])
        ...
        notable_bars = ax2.bar(x - width/2, 
                          [notable_trad/len(reliable_notable)*100,
                           notable_mod/len(reliable_notable)*100,
                           notable_prog/len(reliable_notable)*100],
                          width, label='Notable DAs', color='#e74c3c', alpha=0.7)
        ...
        # Add value labels
        for bars in [notable_bars, state_bars]:
            for bar in bars:
        ...
        top_national = reliable_notable.nlargest(10, 'Mean_Score')
        ...
        ax3.set_title('Top 10 Notable DAs', fontweight='bold', fontsize=13)
        ax3.invert_yaxis()
        ...
        gap = notable_mean - state_mean
        gap_pct = (gap / 3) * 100  # 3 is the range of the scale
        
        prog_count = notable_prog
        mod_count = notable_mod
        trad_count = notable_trad
        
        most_known = reliable_notable.nlargest(1, 'Familiarity_Rate')
        most_known_name = most_known['Name'].values[0]
        most_known_pct = most_known['Familiarity_Rate'].values[0]
        
        avg_unfamiliarity = 100 - reliable_notable['Familiarity_Rate'].mean()
        
        summary_text = f"""
**KEY FINDINGS & OBSERVATIONS:**

**1. Notable vs. State Gap**
   • **Notable DAs:** Rated at **{notable_mean:.2f}** (Progressive-Leaning)
...
**2. Notable DA Ideological Camps**
   • **Progressive (≥3):** {prog_count} DAs ({prog_count/len(reliable_notable):.0%})
   • **Moderate (>2 and <3):** {mod_count} DAs ({mod_count/len(reliable_notable):.0%})
   • **Traditional (≤2):** {trad_count} DAs ({trad_count/len(reliable_notable):.0%})

**3. The Familiarity Factor**
   • The public perception of prosecutors appears
     heavily influenced by a few famous figures.
   • **Average Unfamiliarity** with notable DAs is **{avg_unfamiliarity:.0f}%.**
...
"""
        ...
        plt.suptitle('The Two-Tier System of Prosecutor Perception: Notable vs. State',
                    fontsize=16, fontweight='bold', y=0.98)
        
        plt.savefig(output_path / 'viz_8_notable_vs_state.png', dpi=300, bbox_inches='tight')
        plt.close()
    
    def create_viz_national_vs_state_revised(self, output_path):
        """Create improved 4-panel national vs state comparison visualization"""
        print("   Creating Viz 9: National vs State Comparison (Revised 4-Panel)...")
        ...
        # Get data
        reliable_notable = self.da_df[self.da_df['Substantive_Ratings'] >= self.min_ratings]
        national_mean = reliable_notable['Mean_Score'].mean()
        national_std = reliable_notable['Mean_Score'].std()
        state_mean = self.state_df['mean_score'].mean()
        state_std = self.state_df['mean_score'].std()
        ...
        gap_pct = (abs(gap) / 3) * 100  # Scale range is 3 points (1 to 4)
        ...
        # --- Panel 2: Camp Distribution (Bar Chart) ---
        ax2 = axes[0, 1]
        camps = pd.cut(reliable_notable['Mean_Score'],
                       bins=[0, 2.5, 3.5, 5],
        ...
        for i, (p, num) in enumerate(zip(camp_counts.values, camp_counts)):
            ax2.text(p + 0.1, i, f"{num} DAs ({num/len(reliable_notable)*100:.0f}%)", 
                    va='center', fontsize=11, fontweight='bold')
        
        # --- Panel 3: Weighted vs Unweighted Mean (Familiarity Bias) ---
        ax3 = axes[1, 0]
        weights = reliable_notable['Substantive_Ratings']
        scores = reliable_notable['Mean_Score']
        weighted_mean = np.average(scores, weights=weights)
        unweighted_mean = scores.mean()
        
        means_bias = [weighted_mean, unweighted_mean]
        labels = ['Weighted by # Ratings', 'Unweighted Mean']
        
        bars = ax3.bar(labels, means_bias, color=['#9467bd', '#17becf'], 
        ...
        ax3.set_ylim([max(0, min(means_bias) - 0.2), max(means_bias) + 0.2])
        ax3.grid(axis='y', alpha=0.5, zorder=1)
        
        for bar, val in zip(bars, means_bias):
        ...
        prog_count = (reliable_notable['Mean_Score'] > 3.5).sum()
        mod_count = ((reliable_notable['Mean_Score'] >= 2.5) & (reliable_notable['Mean_Score'] <= 3.5)).sum()
        trad_count = (reliable_notable['Mean_Score'] < 2.5).sum()
...
**2. National DA Ideological Camps**
   • **Progressive (>3.5):** {prog_count} DAs ({prog_count/len(reliable_notable):.0%})
   • **Moderate (2.5-3.5):** {mod_count} DAs ({mod_count/len(reliable_notable):.0%})
   • **Traditional (<2.5):** {trad_count} DAs ({trad_count/len(reliable_notable):.0%})
...
"""
        ...
        plt.savefig(output_path / 'viz_9_national_vs_state_revised.png', dpi=300, bbox_inches='tight')
        plt.close()
...
"""
        ...
        if hasattr(self, 'state_df') and len(self.state_df) > 0:
            notable_mean = reliable['Mean_Score'].mean()
            state_mean = self.state_df['mean_score'].mean()
            
            report += f"""
{'=' * 80}
PART 7: NOTABLE VS STATE COMPARISON
{'=' * 80}

National DAs Mean: {notable_mean:.2f}
State DAs Mean: {state_mean:.2f}
Gap: {notable_mean - state_mean:.2f} points
...
"""
    ...
    def run_complete_analysis(self, output_dir='output'):
        ...
        print("     - viz_1b_progressive_vs_traditional.png")
        print("     - viz_2_all_50_das.png")
        print("     - viz_3_position_effects.png")
        print("     - viz_4_familiarity.png")
        print("     - viz_4b_simple_comparison.png")
        print("     - viz_5a_transitions_bars.png")
        print("     - viz_5b_transitions.png")
        print("     - viz_6_controversy.png")
        print("     - viz_7_geographic.png")
        print("     - viz_8_notable_vs_state.png")
        print("     - viz_9_national_vs_state_revised.png")
        print("\n  📄 Reports:")
