...
5) **Best‑effort hierarchical matcher**:
...
6) Clean exports + a small visualization set (matplotlib‑only, optional).
...
def std_series(s: pd.Series) -> pd.Series:
    return s.astype(str).str.lower().str.replace(r'[^\w\s]', '', regex=True).str.replace(r'\s+', ' ', regex=True).str.strip()


def state_from_jurisdiction(j: str) -> Optional[str]:
    if isinstance(j, str):
        m = re.search(r',\s*([A-Z]{2})$', j.strip())
        if m:
            return Config.ABBR_TO_STATE.get(m.group(1))
    return None


# Nickname map for first‑name compatibility (non‑exhaustive but practical)
NICK_MAP = {
    'bob':'robert','bobby':'robert','rob':'robert','roberto':'robert','robbie':'robert',
    'bill':'william','billy':'william','will':'william','willy':'william','liam':'william',
    'jim':'james','jimmy':'james','jamie':'james',
    'tony':'anthony','antony':'anthony',
    'mike':'michael','mikey':'michael','michael':'michael',
    'liz':'elizabeth','lizzy':'elizabeth','beth':'elizabeth','eliza':'elizabeth','lisa':'elizabeth','elizabeth':'elizabeth',
    'kate':'katherine','katie':'katherine','kathy':'katherine','cathy':'catherine','catherine':'catherine','katherine':'katherine',
    'kim':'kimberly','kimberley':'kimberly','kimmy':'kimberly','kimberly':'kimberly',
    'alex':'alexander','sasha':'alexander','alexander':'alexander','alexandra':'alexandra',
    'rick':'richard','rich':'richard','dick':'richard','richie':'richard',
    'nick':'nicholas','nicky':'nicholas','nicholas':'nicholas',
    'joe':'joseph','joey':'joseph','jose':'jose','joseph':'joseph',
    'ben':'benjamin','benny':'benjamin','benjamin':'benjamin',
    'sam':'samuel','sammy':'samuel','samuel':'samuel',
    'tom':'thomas','tommy':'thomas','thomas':'thomas',
    'harry':'henry','henry':'henry',
    'peggy':'margaret','maggie':'margaret','meg':'margaret','margaret':'margaret',
    'jack':'john','john':'john','jon':'jonathan','jonathan':'jonathan',
    'chris':'christopher','christopher':'christopher','kris':'kristopher'
}


def norm_first(x: str) -> str:
    if not isinstance(x, str) or not x.strip():
        return ''
    x = re.sub(r'[^\w\s]', '', x.lower()).strip()
    tok = x.split()[0]
    return NICK_MAP.get(tok, tok)


def first_name_compatible(a: str, b: str) -> bool:
    """Compatibility for relaxed matching.
...
class DataLoader:
    ...
    def build_prosecutor_list(self):
        ...
        # Combine
        keep_cols = ['q_id','name','fname','lname','state_name','jurisdiction','is_notable','substantive_ratings','familiarity_rate']
...
class ProsecutorAnalyzer:
    ...
    def _prepare_matching_frames(self):
        ...
        df_s['std_dist'] = self._std(df_s['jurisdiction'])
    ...
    def analyze_incumbency(self):
        """Ideology by incumbency (Incumbent/Challenger/Open). Saves PNG + CSV."""
        df = self.df_matched.copy()
        if df.empty or 'incum_chall' not in df.columns:
            print('[incumbency] skipped: no matched data')
            return
        status_map = {'I':'Incumbent','C':'Challenger','O':'Open Seat'}
        df = df[df['incum_chall'].isin(status_map.keys())].copy()
        if df.empty:
            print('[incumbency] skipped: no rows with I/C/O')
            return
        df['status'] = df['incum_chall'].map(status_map)
        # summary CSV
        summ = (df.groupby('status')['mean_score']
                  .agg(n='count', mean='mean', sd='std', median='median')
                  .reset_index())
        out_csv = Config.OUTPUT_DIR / 'ideology_by_incumbency_summary.csv'
        ...
        try:
            ...
            inc = df[df['status']=='Incumbent']['mean_score'].dropna()
            chal = df[df['status']=='Challenger']['mean_score'].dropna()
            if len(inc)>1 and len(chal)>1:
                try:
                    from scipy.stats import mannwhitneyu
                    U,p = mannwhitneyu(inc, chal, alternative='two-sided')
                    print(f'[incumbency] Mann-Whitney U: U={U:.0f}, p={p:.4f}')
                except Exception:
                    pass
        except Exception:
            pass
        ...
        try:
            fig,ax = plt.subplots(figsize=(7,4))
            order = ['Incumbent','Challenger','Open Seat']
            data = [df[df['status']==o]['mean_score'].dropna().values for o in order]
            ax.boxplot(data, labels=order, showfliers=False)
            ax.set_title('Ideology (1=Trad, 4=Prog) by Incumbency')
            ax.set_ylabel('Ideology mean score')
            fig.tight_layout()
            out_png = Config.VIZ_DIR / 'ideology_by_incumbency.png'
            ensure_dir(Config.VIZ_DIR)
            fig.savefig(out_png, dpi=200)
            plt.close(fig)
            print(f'✓ Saved {out_png}')
        except Exception as e:
            print('[incumbency] plot skipped:', e)

    def analyze_contestation(self):
        """Ideology by general election contestation. Saves PNG + CSV."""
        df = self.df_matched.copy()
        if df.empty or 'general_contested_reconciled' not in df.columns:
            print('[contestation] skipped: no matched data')
            return
        df = df[df['general_contested_reconciled'].isin(['contested','uncontested'])].copy()
        if df.empty:
            print('[contestation] skipped: no contested/uncontested label')
            return
        # summary CSV
        summ = (df.groupby('general_contested_reconciled')['mean_score']
                  .agg(n='count', mean='mean', sd='std', median='median')
                  .reset_index())
        out_csv = Config.OUTPUT_DIR / 'ideology_by_contestation_summary.csv'
        ensure_dir(Config.OUTPUT_DIR)
        summ.to_csv(out_csv, index=False)
        print(f'✓ Saved {out_csv}')
        # test (optional)
        try:
            from scipy.stats import mannwhitneyu
            a = df[df['general_contested_reconciled']=='contested']['mean_score'].dropna()
            b = df[df['general_contested_reconciled']=='uncontested']['mean_score'].dropna()
            if len(a)>1 and len(b)>1:
                U,p = mannwhitneyu(a,b, alternative='two-sided')
                print(f'[contestation] Mann-Whitney U: U={U:.0f}, p={p:.4f}')
        except Exception:
            pass
        # plot
        try:
            fig,ax = plt.subplots(figsize=(7,4))
            order = ['contested','uncontested']
            data = [df[df['general_contested_reconciled']==o]['mean_score'].dropna().values for o in order]
            ax.boxplot(data, labels=['Contested','Uncontested'], showfliers=False)
            ax.set_title('Ideology (1=Trad, 4=Prog) by General Election Contestation')
            ax.set_ylabel('Ideology mean score')
            fig.tight_layout()
            out_png = Config.VIZ_DIR / 'ideology_by_contestation.png'
            ensure_dir(Config.VIZ_DIR)
            fig.savefig(out_png, dpi=200)
            plt.close(fig)
            print(f'✓ Saved {out_png}')
        except Exception as e:
            print('[contestation] plot skipped:', e)

    def analyze_recall_risk(self):
        """Toy recall-risk = z(familiarity) * z(ideology). Saves CSV + scatter."""
        df = self.df_prosecutors_filtered.copy()
        if df.empty:
            print('[recall] skipped: no filtered prosecutors')
            return
        # z-scores
        df['familiarity_z'] = (df['familiarity_rate'] - df['familiarity_rate'].mean()) / (df['familiarity_rate'].std(ddof=0) or 1)
        df['ideology_z'] = (df['mean_score'] - df['mean_score'].mean()) / (df['mean_score'].std(ddof=0) or 1)
        df['recall_risk_score'] = df['familiarity_z'] * df['ideology_z']
        top = df.sort_values('recall_risk_score', ascending=False).head(25)
        out_csv = Config.OUTPUT_DIR / 'recall_risk_top25.csv'
        ensure_dir(Config.OUTPUT_DIR)
        top[['name','state_name','jurisdiction','mean_score','familiarity_rate','recall_risk_score']].to_csv(out_csv, index=False)
        print(f'✓ Saved {out_csv}')
        # scatter
        try:
            fig,ax = plt.subplots(figsize=(6,6))
            ax.scatter(df['mean_score'], df['familiarity_rate'])
            ax.set_xlabel('Ideology mean score (1–4)')
            ax.set_ylabel('Familiarity (%)')
            ax.set_title('Familiarity vs Ideology (toy recall-risk gradient)')
            fig.tight_layout()
            out_png = Config.VIZ_DIR / 'recall_risk_scatter.png'
            ensure_dir(Config.VIZ_DIR)
            fig.savefig(out_png, dpi=200)
            plt.close(fig)
            print(f'✓ Saved {out_png}')
        except Exception as e:
            print('[recall] plot skipped:', e)

    def analyze_close_victory_model(self):
        """Logit: Pr(close<55%) ~ ideology + contested + incumbent + familiarity. Saves summary."""
        df = self.df_matched_all.copy()
        if df.empty:
            print('[close-victory] skipped: no matched data')
            return
        # features
        df['vote_share'] = pd.to_numeric(df.get('vote_percent_general'), errors='coerce')
        df['close_bin'] = (df['vote_share'] < 55).astype('float')
        df['is_contested'] = (df.get('general_contested_reconciled') == 'contested').astype('float')
        df['is_incumbent'] = (df.get('incum_chall') == 'I').astype('float')
        mdat = df.dropna(subset=['close_bin','mean_score','is_contested','is_incumbent','familiarity_rate'])
        if len(mdat) < 20:
            print('[close-victory] skipped: insufficient rows')
            return
        ensure_dir(Config.OUTPUT_DIR)
        out_txt = Config.OUTPUT_DIR / 'close_victory_logit_summary.txt'
        try:
            import statsmodels.formula.api as smf
            model = smf.logit('close_bin ~ mean_score + is_contested + is_incumbent + familiarity_rate', data=mdat).fit(disp=0)
            open(out_txt,'w').write(str(model.summary()))
            print(f'✓ Saved {out_txt}')
        except Exception as e:
            # fallback: OLS LPM
            try:
                import statsmodels.formula.api as smf
                model = smf.ols('close_bin ~ mean_score + is_contested + is_incumbent + familiarity_rate', data=mdat).fit()
                open(out_txt,'w').write('LOGIT unavailable; OLS fallback
' + str(model.summary()))
                print(f'✓ Saved {out_txt} (OLS fallback)')
            except Exception as e2:
                print('[close-victory] both LOGIT/OLS unavailable:', e2)
    ...
    def export(self):
        ...
        audit_cols = [
            'name','fname','lname','state_name','jurisdiction','is_notable','substantive_ratings','familiarity_rate',
        ...
        hits_all = int(self.df_matched_all['election_year'].notna().sum())
        hits_f = int(self.df_matched['election_year'].notna().sum())
        lines.append(f"Matched (ALL): {hits_all}/{len(self.df_matched_all)} ({hits_all/len(self.df_matched_all)*100:.1f}%)")
        lines.append(f"Matched (FILTERED): {hits_f}/{len(self.df_matched)} ({hits_f/max(1,len(self.df_matched))*100:.1f}%)")
        lines.append('')
        lines.append('Files:')
        for p in [p_all,p_filt,p_match_all,p_match_f,p_audit]:
            lines.append(f" - {p}")
        lines.append(f" - {Config.VIZ_DIR}/*.png")
...
def main():
    ...
    if not Path(Config.SURVEY_FILE).exists():
        print(f"✗ Missing survey file: {Config.SURVEY_FILE}")
        return
    if not Path(Config.ELECTION_FILE).exists():
        print(f"✗ Missing elections file: {Config.ELECTION_FILE}")
        return
    ...
    # Optional viz/models
    try:
        an.analyze_familiarity()
    except Exception as e:
        print('[viz] skipped:', e)
    try:
        an.run_models()
    except Exception as e:
        print('[models] skipped:', e)
    # New modules for end-to-end reproducibility
    try:
        an.analyze_incumbency()
    except Exception as e:
        print('[incumbency] skipped:', e)
    try:
        an.analyze_contestation()
    except Exception as e:
        print('[contestation] skipped:', e)
    try:
        an.analyze_recall_risk()
    except Exception as e:
        print('[recall] skipped:', e)
    try:
        an.analyze_close_victory_model()
    except Exception as e:
        print('[close-victory] skipped:', e)
