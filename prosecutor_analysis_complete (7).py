...
class ProsecutorAnalyzer:
    ...
    def run_all_analyses(self):
        ...
        # Print sample sizes for transparency
        print(f"\nSample sizes:")
        print(f"  All prosecutors: {len(self.df_prosecutors)}")
        print(f"  Filtered (≥{Config.MIN_RATINGS_THRESHOLD} ratings): {len(self.df_prosecutors_filtered)}")
        print(f"  Full matched (with election data): {len(self.df_matched_all)}")
        print(f"  Filtered + matched: {len(self.df_matched)}")
        
        notable_matched = self.df_matched_all[self.df_matched_all['is_notable'] == True]
        state_matched = self.df_matched_all[self.df_matched_all['is_notable'] == False]
        print(f"    → Notable with election data: {len(notable_matched)}")
        print(f"    → State with election data: {len(state_matched)}")
        print()
    ...
    def _analyze_familiarity_comparisons(self):
        ...
        notable = self.df_prosecutors[self.df_prosecutors['is_notable'] == True]['familiarity_rate']
        state = self.df_prosecutors[self.df_prosecutors['is_notable'] == False]['familiarity_rate']
    ...
    def _analyze_electoral_competition(self):
        ...
        contested = df[df['general_contested_reconciled'] == 'contested']['familiarity_rate']
        uncontested = df[df['general_contested_reconciled'] == 'uncontested']['familiarity_rate']
        ...
        for notable_val, label in [(True, 'Notable'), (False, 'State')]:
            ...
            if len(subset) > 10:
                contested_sub = subset[subset['general_contested_reconciled'] == 'contested']['familiarity_rate']
                uncontested_sub = subset[subset['general_contested_reconciled'] == 'uncontested']['familiarity_rate']
                
                if len(contested_sub) > 0 and len(uncontested_sub) > 0:
